...
<article id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> clearfix"<?php print $attributes; ?>> <?php print $user_picture; ?> <?php print render($title_prefix); ?>
   ...
   <div class="audio-wrapper">
    ...
    <script type="text/javascript">
      (function($){var repeat=localStorage.repeat||0,shuffle=localStorage.shuffle||'false',continous=true,autoplay=true,playlist=window.playlist;for(var i=0;i<playlist.length;i++){var item=playlist[i];$('#playlist').append('<li>'+item.artist+' - '+item.title+'</li>');}
        ...
        loadMusic(currentTrack);$('.play-control button').on('click',function(){console.log('click');if($(this).parents('.audio').hasClass('state-playing')){$(this).parents('.audio').removeClass('state-playing');pause();}else{$(this).parents('.audio').addClass('state-playing');play();}});$('.rewind').on('click',function(){if(shuffle==='true'){shufflePlay();}else{switchTrack(--currentTrack);}});$('.fastforward').on('click',function(){if(shuffle==='true'){shufflePlay();}else{switchTrack(++currentTrack);}});$('#playlist li').each(function(i){var _i=i;$(this).on('click',function(){switchTrack(_i);});});if(shuffle==='true')$('.shuffle').addClass('enable');if(repeat==1){$('.repeat').addClass('once');}else if(repeat==2){$('.repeat').addClass('all');}
  ...
  <?php if($view_mode == 'full'):?>
    <?php 
    	$relate_node = dale_prev_next_node($node, array('article'));
    	if($relate_node['prev']){
