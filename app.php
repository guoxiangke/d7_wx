...
function liangyou_audio_list_byindex(){
  ...
  foreach ($audios as $code => $audio) {
    $audios[$code]['code'] = $code;
    unset($audios[$code]['bce']);
    unset($audios[$code]['txly']);
    unset($audios[$code]['prefix']);
    unset($audios[$code]['prefix']);
...
// $apps = liangyou_audio_list_byindex();

// $category = array_column($apps, 'category');
// // var_dump($category);
// $categorys = array_unique($category);
...
//   file_put_contents( 'appjson/'.$category_id.'.json' , $temp );
// }
//
$temp = array('version'=>'1.0','update'=>time());
$temp = json_encode($temp);
file_put_contents( 'appjson/update.json' , $temp );
